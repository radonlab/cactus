...
static void fs_rmdir_helper(WCHAR* _wsource) {
...
static int fs_rm_wait(WCHAR* _wpath) {
  unsigned retries = 1;
  DWORD last_error;
...
static void fs_rm(const char* _source) {
  ...
  if (INVALID_FILE_ATTRIBUTES == attrs) return;
...
static void fs_copydir_helper(WCHAR* _wsource, WCHAR* _wdest) {
...
static void fs_copy(const char* _source, const char* _dest) {
...
void cl_fs_cleanup(void) { fs_rm(fixture_path(_clar_path, "*")); }
...
static void basename_r(const char** out, int* out_len, const char* in) {
  ...
  for (in_len = strlen(in); in_len; in_len--) {
    if (in[in_len - 1] != '/') break;
  }

  for (start_pos = in_len; start_pos; start_pos--) {
    if (in[start_pos - 1] == '/') break;
  }
...
static void fs_copy(const char* source, const char* _dest) {
  char* dbuf = NULL;
...
static void fs_rmdir_helper(const char* path) {
  DIR* dir;
  struct dirent* d;
...
static void fs_rm(const char* path) {
  struct stat st;

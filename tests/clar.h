...
typedef void(cl_trace_cb)(cl_trace_event ev, const char* suite_name,
                          const char* test_name, void* payload);
...
#define cl_must_pass_(expr, desc)                         \
  clar__assert((expr) >= 0, __FILE__, __func__, __LINE__, \
               "Function call failed: " #expr, desc, 1)
#define cl_must_fail_(expr, desc)                        \
  clar__assert((expr) < 0, __FILE__, __func__, __LINE__, \
               "Expected function call to fail: " #expr, desc, 1)
#define cl_assert_(expr, desc)                            \
  clar__assert((expr) != 0, __FILE__, __func__, __LINE__, \
               "Expression is not true: " #expr, desc, 1)
...
#define cl_check_pass_(expr, desc)                        \
  clar__assert((expr) >= 0, __FILE__, __func__, __LINE__, \
               "Function call failed: " #expr, desc, 0)
#define cl_check_fail_(expr, desc)                       \
  clar__assert((expr) < 0, __FILE__, __func__, __LINE__, \
               "Expected function call to fail: " #expr, desc, 0)
#define cl_check_(expr, desc)                             \
  clar__assert((expr) != 0, __FILE__, __func__, __LINE__, \
               "Expression is not true: " #expr, desc, 0)
...
#define cl_warning(desc)                   \
  clar__fail(__FILE__, __func__, __LINE__, \
             "Warning during test execution:", desc, 0)
...
#define cl_assert_equal_s(s1, s2)                  \
  clar__assert_equal(__FILE__, __func__, __LINE__, \
                     "String mismatch: " #s1 " != " #s2, 1, "%s", (s1), (s2))
#define cl_assert_equal_s_(s1, s2, note)                                   \
  clar__assert_equal(__FILE__, __func__, __LINE__,                         \
                     "String mismatch: " #s1 " != " #s2 " (" #note ")", 1, \
                     "%s", (s1), (s2))

#define cl_assert_equal_wcs(wcs1, wcs2)                                        \
  clar__assert_equal(__FILE__, __func__, __LINE__,                             \
                     "String mismatch: " #wcs1 " != " #wcs2, 1, "%ls", (wcs1), \
                     (wcs2))
#define cl_assert_equal_wcs_(wcs1, wcs2, note)                                 \
  clar__assert_equal(__FILE__, __func__, __LINE__,                             \
                     "String mismatch: " #wcs1 " != " #wcs2 " (" #note ")", 1, \
                     "%ls", (wcs1), (wcs2))

#define cl_assert_equal_strn(s1, s2, len)                                 \
  clar__assert_equal(__FILE__, __func__, __LINE__,                        \
                     "String mismatch: " #s1 " != " #s2, 1, "%.*s", (s1), \
                     (s2), (int)(len))
...
#define cl_assert_equal_wcsn(wcs1, wcs2, len)                            \
  clar__assert_equal(__FILE__, __func__, __LINE__,                       \
                     "String mismatch: " #wcs1 " != " #wcs2, 1, "%.*ls", \
                     (wcs1), (wcs2), (int)(len))
...
#define cl_assert_equal_p(p1, p2)                  \
  clar__assert_equal(__FILE__, __func__, __LINE__, \
                     "Pointer mismatch: " #p1 " != " #p2, 1, "%p", (p1), (p2))

void clar__skip(void);

void clar__fail(const char* file, const char* func, size_t line,
                const char* error, const char* description, int should_abort);

void clar__assert(int condition, const char* file, const char* func,
                  size_t line, const char* error, const char* description,
                  int should_abort);

void clar__assert_equal(const char* file, const char* func, size_t line,

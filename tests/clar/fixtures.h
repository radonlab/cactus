static const char* fixture_path(const char* base, const char* fixture_name) {
  ...
  if (_path[root_len - 1] != '/') _path[root_len++] = '/';

  if (fixture_name[0] == '/') fixture_name++;

  strncpy(_path + root_len, fixture_name, sizeof(_path) - root_len);
...
const char* cl_fixture_basename(const char* fixture_name) {
  ...
  for (p = fixture_name; *p; p++) {
    if (p[0] == '/' && p[1] && p[1] != '/') fixture_name = p + 1;
  }

...
static size_t buf_istream_read(cactus_buf_istream_t* stream, uint8_t* buf, size_t len) {
  return len;
}

static bool buf_istream_seek(cactus_buf_istream_t* stream, size_t pos) {
  return true;
}

static size_t buf_ostream_write(cactus_buf_ostream_t* stream, const uint8_t* buf, size_t len) {
  return len;
}

...
/* Core test functions */
static void clar_report_errors(struct clar_report* report) {
...
static void clar_run_test(const struct clar_func* test,
  ...
  if (setjmp(_clar.trampoline) == 0) {
    if (initialize->ptr != NULL) initialize->ptr();
...
static void clar_run_suite(const struct clar_suite* suite, const char* filter) {
  ...
  for (i = 0; i < suite->test_count; ++i) {
    if (filter && strncmp(test[i].name, filter, matchlen)) continue;
    ...
    if (_clar.reports == NULL) _clar.reports = report;

    if (_clar.last_report != NULL) _clar.last_report->next = report;
...
static void clar_usage(const char* arg) {
  printf("Usage: %s [options]\n\n", arg);
  printf("Options:\n");
  printf(
      "  -sname        Run only the suite with `name` (can go to individual "
      "test name)\n");
...
static void clar_parse_args(int argc, char** argv) {
  int i;
      ...
      case 'x': { /* given suite name */
        ...
        for (j = 0; j < _clar_suite_count; ++j) {
          ...
          if (strncmp(argument, _clar_suites[j].name, cmplen) == 0) {
            ...
            if (!exact) _clar.report_suite_names = 1;
...
void clar_test_init(int argc, char** argv) {
  if (argc > 1) clar_parse_args(argc, argv);
...
static void abort_test(void) {
  if (!_clar.trampoline_enabled) {
    clar_print_onabort("Fatal error: a cleanup method raised an exception.");
    clar_report_errors(_clar.last_report);
    exit(-1);
  }
...
void clar__assert_equal(const char* file, const char* function, size_t line,
  ...
  if (!strcmp("%s", fmt)) {
    ...
    is_equal = (p1 == p2);
    if (!is_equal) p_snprintf(buf, sizeof(buf), "%p != %p", p1, p2);
  } else {
  ...
  if (!is_equal) clar__fail(file, function, line, err, buf, should_abort);
}

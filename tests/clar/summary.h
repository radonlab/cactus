...
int clar_summary_testsuite(struct clar_summary* summary, int idn, const char* name, const char* pkg, time_t timestamp,
  ...
  if (strftime(iso_dt, sizeof(iso_dt), "%Y-%m-%dT%H:%M:%S", tm) == 0) return -1;
...
int clar_summary_shutdown(struct clar_summary* summary) {
  ...
  if (clar_summary_testsuites(summary) < 0) goto on_error;
  ...
  while (report != NULL) {
    ...
    if (clar_summary_close_tag(summary, "testcase", 2) < 0) goto on_error;

    report = report->next;

    if (!report || strcmp(last_suite, report->suite) != 0) {
      if (clar_summary_close_tag(summary, "testsuite", 1) < 0) goto on_error;
    }
  }
